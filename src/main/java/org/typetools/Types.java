...
public final class Types {
    ...
    public static <T> Class<T> resolveClass(Type type) {
        return type instanceof Class ? (Class<T>) type
                : type instanceof ParameterizedType ? (Class<T>) ((ParameterizedType) type)
                        .getRawType() : null;
    }
    ...
    public static <T, I extends T> Class<?>[] resolveArguments(Class<T> target, Class<I> initialType) {
        Type[] typeArguments = resolveArguments(initialType, target, target.isInterface(), null,
                null);
    ...
    private static Type[] resolveArguments(Type subject, Class<?> rawTarget,
            boolean resolveInterfaces, Class<?> previousDeclaration,
            ParameterizedType previousInstance) {
        ...
        if (superType != null && !superType.equals(Object.class)) {
            Type[] typeArguments = resolveArguments(superType, rawTarget, resolveInterfaces,
        ...
        if (resolveInterfaces)
            for (Type interfaceType : rawSubject.getGenericInterfaces()) {
                Type[] typeArguments = resolveArguments(interfaceType, rawTarget,
                        resolveInterfaces, rawSubject, parameterizedSubject);
                if (typeArguments != null)
                    return bindArguments(typeArguments, previousDeclaration, previousInstance);
            }
